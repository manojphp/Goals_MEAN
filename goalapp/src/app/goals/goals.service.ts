...
export class GoalsService implements OnInit{
  ...
  getGoals() {
    return this.http.get<any>(environment.server + '/goals/user/'+this.userid);
  }
  ...
  addGoal(formdata:any) {
    ...
    return this.http.post(environment.server + '/goals/user/'+this.userid, formdata);
  }
  ...
  deleteGoalById(goal_id: string) {
    return this.http.delete(environment.server + '/goals/user/'+this.userid+'/goal/' + goal_id);
  }
}
